...
local function iWsam(msg,MsgText)
...
if not redis:get(wsam..'group:add'..msg.chat_id_) then return '🛠*¦* المجموعه بالتأكيد ✓️ تم تعطيلها' end  
...
if redis:get(wsam..'lock_id'..msg.chat_id_) then
GetUserID(msg.sender_user_id_,function(arg,data)
local msgs = redis:get(wsam..'msgs:'..data.id_..':'..arg.chat_id_) or 1
if data.username_ then UserNameID = "🎫¦ مـعرفك •⊱ @"..data.username_.." ⊰•\n" else UserNameID = "" end
if data.username_ then UserNameWsam = '@'..data.username_ else UserNameWsam = 'لا يوجد' end
if data.last_name_ then Name = data.first_name_ .." "..data.last_name_ else Name = data.first_name_ end
local Namei = FlterName(data,20)
local NumGhaWsam = (redis:get(wsam..':adduser:'..msg.chat_id_..':'..msg.sender_user_id_) or 0)
local editedwsam = (redis:get(wsam..':edited:'..msg.chat_id_..':'..msg.sender_user_id_) or 0)
local gameswsam = (redis:get(wsam..':User_Points:'..msg.chat_id_..msg.sender_user_id_) or 0)
if not redis:get(wsam..':lock_id_photo:'..arg.chat_id_) then
IdWsamFagehi = (redis:get(wsam..'idwsam:msg'..msg.chat_id_) or 
'👤¦ أســمـك •⊱ { '..Namei..' } ⊰•\n'
..'['..UserNameID..']'
..'🏷¦ ايديــك •⊱ {`'..data.id_..'`} ⊰•\n\n'
...
..'💬¦ رسائلك •⊱ {'..msgs..'} ⊰•\n➖')
local IdWsamFagehi = IdWsamFagehi:gsub("{الاسم}",Namei)
local IdWsamFagehi = IdWsamFagehi:gsub("{المعرف}",UserNameWsam)
IdWsamFagehi = IdWsamFagehi:gsub("{الايدي}",data.id_)
IdWsamFagehi = IdWsamFagehi:gsub("{الرتبه}",arg.TheRank)
IdWsamFagehi = IdWsamFagehi:gsub("{التفاعل}",Get_Ttl(msgs))
IdWsamFagehi = IdWsamFagehi:gsub("{الرسائل}",msgs)
IdWsamFagehi = IdWsamFagehi:gsub("{الجهات}",NumGhawsam)
IdWsamFagehi = IdWsamFagehi:gsub("{السحكات}",editedwsam)
IdWsamFagehi = IdWsamFagehi:gsub("{النقاط}",gameswsam)
sendMsg(arg.chat_id_,arg.id_,IdWsamFagehi)
return false
end
...
..'💬¦ رسائلك •⊱ {'..arg.msgs..'} ⊰•\n'
..'🖲¦ نقاطك •⊱ {'..gameswsam..'} ⊰•\n➖')
local IdWsamFagehi = IdWsamFagehi:gsub("{الاسم}",arg.Namei)
local IdWsamFagehi = IdWsamFagehi:gsub("{المعرف}",UserNamewsam)
IdWsamFagehi = IdWsamFagehi:gsub("{الايدي}",arg.sender_user_id_)
IdWsamFagehi = IdWsamFagehi:gsub("{الرتبه}",arg.TheRank)
IdWsamFagehi = IdWsamFagehi:gsub("{التفاعل}",Get_Ttl(arg.msgs))
IdWsamFagehi = IdWsamFagehi:gsub("{الرسائل}",arg.msgs)
IdWsamFagehi = IdWsamFagehi:gsub("{الجهات}",NumGhawsam)
IdWsamFagehi = IdWsamFagehi:gsub("{السحكات}",editedwsam)
IdWsamFagehi = IdWsamFagehi:gsub("{النقاط}",gameswsam)
sendPhoto(arg.chat_id_,arg.id_,data.photos_[0].sizes_[1].photo_.persistent_id_,IdWsamFagehi,dl_cb,nil)
else
IdWsamFagehi = (redis:get(wsam..'idwsam:msg'..msg.chat_id_) or 
...
end,{chat_id_=arg.chat_id_,id_=arg.id_,TheRank=arg.TheRank,sender_user_id_=data.id_,msgs=msgs,Namei=Namei,UserNameID=UserNameID,NumGhaWsam=NumGhaWsam,editedwsam=editedwsam})
end
...
local GroupName = (redis:get(wsam..'group:name'..arg.ChatID) or '')
redis:set(wsam..'linkGroup'..arg.ChatID,(data.invite_link_ or ""))
...
if not msg.Director and redis:get(wsam..'lock_pin'..msg.chat_id_) then
...
function(arg,data)
if data.ID == "Ok" then
redis:set(wsam..":MsgIDPin:"..arg.ChatID,arg.reply_id)
...
if not msg.Admin then return "📛*¦* هذا الامر يخص {الادمن,المدير,المنشئ,المطور} فقط  \n🚶" end
if not msg.Director and redis:get(wsam..'lock_pin'..msg.chat_id_) then return "لا يمكنك الغاء التثبيت الامر مقفول من قبل الاداره" end
...
elseif redis:sismember(wsam..':SUDO_BOT:',UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"👤*¦* لا يمكنك تقييد المطور\n🛠") 
elseif redis:sismember(wsam..':MONSHA_BOT:'..arg.ChatID,UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"👤*¦* لا يمكنك تقييد المنشئ\n🛠") 
elseif redis:sismember(wsam..'owners:'..arg.ChatID,UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"👤*¦* لا يمكنك تقييد المدير\n🛠") 
elseif redis:sismember(wsam..'admins:'..arg.ChatID,UserID) then 
...
USERNAME = USERNAME:gsub([[\_]],"_")
USERCAR = utf8.len(USERNAME) 
if redis:sismember(wsam..'whitelist:'..arg.ChatID,arg.UserID) then 
SendMention(arg.ChatID,arg.UserID,arg.MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..arg.UserID..'}\n🛠¦ انه بالتأكيد عضو مميز \n✓️',12,utf8.len(USERNAME))
else
redis:srem(wsam..'is_silent_users:'..arg.ChatID,arg.UserID)
redis:srem(wsam..'banned:'..arg.ChatID,arg.UserID)
redis:hset(wsam..'username:'..arg.UserID,'username',USERNAME)
redis:sadd(wsam..'whitelist:'..arg.ChatID,arg.UserID)
...
UserName = Flter_Markdown(arg.UserName)
if redis:sismember(wsam..'whitelist:'..arg.ChatID,UserID) then 
sendMsg(arg.ChatID,arg.MsgID,'👤*¦* العضو » '..UserName..'\n🎫*¦* الايدي » (`'..UserID..'`)\n🛠*¦* انه بالتأكيد من عضو مميز \n✓️') 
else
redis:srem(wsam..'is_silent_users:'..arg.ChatID,UserID)
redis:srem(wsam..'banned:'..arg.ChatID,UserID)
redis:hset(wsam..'username:'..UserID,'username',UserName)
redis:sadd(wsam..'whitelist:'..arg.ChatID,UserID)
...
USERNAME = USERNAME:gsub([[\_]],"_")
USERCAR = utf8.len(USERNAME) 
if not redis:sismember(wsam..'whitelist:'..arg.ChatID,arg.UserID) then 
SendMention(arg.ChatID,arg.UserID,arg.MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..arg.UserID..'}\n🛠¦ انه بالتأكيد ليس عضو مميز \n✓️',12,utf8.len(USERNAME)) 
else
redis:srem(wsam..'whitelist:'..arg.ChatID,arg.UserID)
...
local UserID = data.id_
UserName = Flter_Markdown(arg.UserName)
if not redis:sismember(wsam..'whitelist:'..arg.ChatID,UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,'👤*¦* العضو » '..UserName..'\n🎫*¦* الايدي » (`'..UserID..'`)\n🛠*¦* انه بالتأكيد ليس من عضو مميز \n✓️') 
else
redis:srem(wsam..'whitelist:'..arg.ChatID,UserID)
return sendMsg(arg.ChatID,arg.MsgID,'👤*¦* العضو » '..UserName..' \n🎫*¦* الايدي » (`'..UserID..'`)\n🛠*¦* تمت تنزيله من عضو مميز \n✓️')
end
end,{ChatID=msg.chat_id_,MsgID=msg.id_,UserName=MsgText[2]})
elseif MsgText[2] and MsgText[2]:match('^%d+$') then
...
USERNAME = USERNAME:gsub([[\_]],"_")
USERCAR = utf8.len(USERNAME) 
if redis:sismember(wsam..'owners:'..arg.ChatID,arg.UserID) then 
SendMention(arg.ChatID,arg.UserID,arg.MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..arg.UserID..'}\n🛠¦ انه بالتأكيد مدير \n✓️',12,utf8.len(USERNAME)) 
else
redis:srem(wsam..'is_silent_users:'..arg.ChatID,arg.UserID)
redis:srem(wsam..'banned:'..arg.ChatID,arg.UserID)
redis:srem(wsam..'whitelist:'..arg.ChatID,arg.UserID)
redis:srem(wsam..'admins:'..arg.ChatID,arg.UserID)
redis:srem(wsam..'banned:'..arg.ChatID,arg.UserID)
redis:hset(wsam..'username:'..arg.UserID,'username',USERNAME)
redis:sadd(wsam..'owners:'..arg.ChatID,UserID)
...
UserName = Flter_Markdown(arg.UserName)
if redis:sismember(wsam..'owners:'..arg.ChatID,UserID) then 
sendMsg(arg.ChatID,arg.MsgID,'👤*¦* العضو » '..UserName..'\n🎫*¦* الايدي » (`'..UserID..'`)\n🛠*¦* انه بالتأكيد مدير \n✓️') 
else
redis:hset(wsam..'username:'..UserID, 'username',UserName)
redis:sadd(wsam..'owners:'..arg.ChatID,UserID)
...
redis:srem(wsam..'banned:'..arg.ChatID,UserID)
redis:srem(wsam..'whitelist:'..arg.ChatID,UserID)
redis:srem(wsam..'admins:'..arg.ChatID,UserID)
redis:srem(wsam..'banned:'..arg.ChatID,arg.UserID)
end,{ChatID=msg.chat_id_,MsgID=msg.id_,UserName=MsgText[2]})
elseif MsgText[2] and MsgText[2]:match('^%d+$') then
...
USERNAME = USERNAME:gsub([[\_]],"_")
USERCAR = utf8.len(USERNAME) 
if not redis:sismember(wsam..'owners:'..arg.ChatID,arg.UserID) then 
...
redis:srem(wsam..'admins:'..arg.ChatID,arg.UserID)
redis:srem(wsam..'banned:'..arg.ChatID,arg.UserID)
redis:srem(wsam..'owners:'..arg.ChatID,arg.UserID)
SendMention(arg.ChatID,arg.UserID,arg.MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..arg.UserID..'}\n🛠¦ تم تنزيله من المدير  \n✓️',12,utf8.len(USERNAME)) 
end
...
local UserID = data.id_
UserName = Flter_Markdown(arg.UserName)
if not redis:sismember(wsam..'owners:'..arg.ChatID,UserID) then 
sendMsg(arg.ChatID,arg.MsgID,'👤*¦* العضو » '..UserName..' \n🎫*¦* الايدي » (`'..UserID..'`)\n🛠*¦* انه بالتأكيد ليس مدير \n✓️') 
else
redis:srem(wsam..'owners:'..arg.ChatID,UserID)
...
USERNAME = USERNAME:gsub([[\_]],"_")
USERCAR = utf8.len(USERNAME) 
if redis:sismember(wsam..':MONSHA_BOT:'..arg.ChatID,arg.UserID) then 
SendMention(arg.ChatID,arg.UserID,arg.MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..arg.UserID..'}\n🛠¦ انه بالتأكيد منشىء  \n✓️',12,utf8.len(USERNAME)) 
else
redis:srem(wsam..'is_silent_users:'..arg.ChatID,arg.UserID)
redis:srem(wsam..'banned:'..arg.ChatID,arg.UserID)
...
redis:srem(wsam..'owners:'..arg.ChatID,arg.UserID)
redis:hset(wsam..'username:'..arg.UserID,'username',USERNAME)
redis:sadd(wsam..':MONSHA_BOT:'..arg.ChatID,arg.UserID)
...
UserName = Flter_Markdown(arg.UserName)
if redis:sismember(wsam..':MONSHA_BOT:'..arg.ChatID,UserID) then 
...
redis:srem(wsam..'banned:'..arg.ChatID,UserID)
redis:srem(wsam..'owners:'..arg.ChatID,UserID)
redis:sadd(wsam..':MONSHA_BOT:'..arg.ChatID,UserID)
sendMsg(arg.ChatID,arg.MsgID,'👤*¦* العضو » '..UserName..' \n🎫*¦* الايدي » (`'..UserID..'`)\n🛠*¦* تمت ترقيته ليصبح منشىء في البوت \n✓️')
end
...
USERNAME = USERNAME:gsub([[\_]],"_")
USERCAR = utf8.len(USERNAME) 
if not redis:sismember(wsam..':MONSHA_BOT:'..arg.ChatID,arg.UserID) then
...
redis:srem(wsam..'banned:'..arg.ChatID,arg.UserID)
redis:srem(wsam..'owners:'..arg.ChatID,arg.UserID)
redis:srem(wsam..':MONSHA_BOT:'..arg.ChatID,arg.UserID)
...
local UserID = data.id_
UserName = Flter_Markdown(arg.UserName)
if not redis:sismember(wsam..':MONSHA_BOT:'..arg.ChatID,UserID) then 
sendMsg(arg.ChatID,arg.MsgID,'👤*¦* العضو » '..UserName..'\n🎫*¦* الايدي » (`'..UserID..'`)\n🛠*¦* انه بالتأكيد ليس منشىء \n✓️') 
else
redis:srem(wsam..'is_silent_users:'..arg.ChatID,UserID)
redis:srem(wsam..'banned:'..arg.ChatID,UserID)
...
redis:srem(wsam..'banned:'..arg.ChatID,UserID)
redis:srem(wsam..'owners:'..arg.ChatID,UserID)
redis:srem(wsam..':MONSHA_BOT:'..arg.ChatID,UserID)
...
USERNAME = USERNAME:gsub([[\_]],"_")
USERCAR = utf8.len(USERNAME) 
if redis:sismember(wsam..'admins:'..arg.ChatID,arg.UserID) then 
SendMention(arg.ChatID,arg.UserID,arg.MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..arg.UserID..'}\n🛠¦ انه بالتأكيد ادمن \n✓️',12,utf8.len(USERNAME)) 
else
redis:srem(wsam..'is_silent_users:'..arg.ChatID,arg.UserID)
redis:srem(wsam..'banned:'..arg.ChatID,arg.UserID)
...
redis:srem(wsam..'banned:'..arg.ChatID,arg.UserID)
redis:hset(wsam..'username:'..arg.UserID,'username',USERNAME)
redis:sadd(wsam..'admins:'..arg.ChatID,arg.UserID)
...
UserName = Flter_Markdown(arg.UserName)
if redis:sismember(wsam..'admins:'..arg.ChatID,UserID) then 
sendMsg(arg.ChatID,arg.MsgID,'👤*¦* العضو » '..UserName..' \n🎫*¦* الايدي » (`'..UserID..'`)\n🛠*¦* انه بالتأكيد ادمن \n✓️') 
else
redis:srem(wsam..'is_silent_users:'..arg.ChatID,UserID)
redis:srem(wsam..'banned:'..arg.ChatID,UserID)
...
redis:srem(wsam..'banned:'..arg.ChatID,UserID)
redis:hset(wsam..'username:'..UserID,'username',UserName)
redis:sadd(wsam..'admins:'..arg.ChatID,UserID)
sendMsg(arg.ChatID,arg.MsgID,'👤*¦* العضو » '..UserName..' \n🎫*¦* الايدي » (`'..UserID..'`)\n🛠*¦* تمت ترقيته ليصبح ادمن \n✓️')
end
end,{ChatID=msg.chat_id_,MsgID=msg.id_,UserName=MsgText[2]})
elseif MsgText[2] and MsgText[2]:match('^%d+$') then
...
USERNAME = USERNAME:gsub([[\_]],"_")
USERCAR = utf8.len(USERNAME) 
if not redis:sismember(wsam..'admins:'..arg.ChatID,arg.UserID) then 
...
local UserID = data.id_
UserName = Flter_Markdown(arg.UserName)
if not redis:sismember(wsam..'admins:'..arg.ChatID,UserID) then 
sendMsg(arg.ChatID,arg.MsgID,'👤*¦* العضو » '..UserName..' \n🎫*¦* الايدي » (`'..UserID..'`)\n🛠*¦* انه بالتأكيد ليس ادمن \n✓️') 
else
redis:srem(wsam..'admins:'..arg.ChatID,UserID)
sendMsg(arg.ChatID,arg.MsgID,'👤*¦* العضو » '..UserName..' \n🎫*¦* الايدي » (`'..UserID..'`)\n🛠*¦* تم تنزيله من الادمنيه \n✓️')
...
local maseegs = redis:get(wsam..'msgs:'..arg.UserID..':'..arg.ChatID) or 1
USERNAME = USERNAME:gsub([[\_]],"_")
USERCAR = utf8.len(USERNAME) 
...
if not data.id_ then return sendMsg(arg.ChatID,arg.MsgID,"📛*¦* لآ يوجد عضـو بهہ‌‏ذآ آلمـعرف \n❕") end 
local UserID = data.id_
local maseegs = redis:get(wsam..'msgs:'..UserID..':'..ChatID) or 1
...
if not msg.Creator then return "📛*¦*هذا الامر يخص {المطور,المنشئ} فقط  \n🚶‍♂️" end
local WsamChatIdG = MsgText[2]
GetFullChat(WsamChatIdG,function(arg,data)
if not data.member_count_ then return sendMsg(msg.chat_id_,arg.MsgID,"📛¦ فشل التحقق من المجموعة\n❕") end
local GroupName = (redis:get(wsam..'group:name'..arg.ChatID) or '')
local LinkGbWsam = data.invite_link_
if not LinkGbWsam then
local LinkGbWsam = ExportLink(WsamChatIdG)
if LinkGbWsam then
LinkGbWsam = LinkGbWsam.result
end
end
...
.."\n*🗣¦* عدد الادمـنـيـه •⊱ { *"..data.administrator_count_.."* } ⊰•"
.."\n*🔚¦* الايــدي •⊱ { `"..arg.ChatID.."` } ⊰•"
.."\n\nـ •⊱ {  ["..FlterName(GroupName).."]("..(LinkGbWsam or "")..")  } ⊰•\n"
)
end,{ChatID=WsamChatIdG,MsgID=msg.id_}) 
return false
end
end
...
USERCAR = utf8.len(USERNAME) 
SendMention(arg.ChatID,arg.UserID,arg.MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..arg.UserID..'}\n🛠¦ تم طرد العضو \n✓️',12,utf8.len(USERNAME))
redis:sadd(wsam..'banned:'..arg.ChatID,arg.UserID)
...
redis:hset(wsam..'username:'..arg.UserID,'username',arg.UserName)
redis:sadd(wsam..'banned:'..arg.ChatID,arg.UserID)
StatusLeft(arg.ChatID,arg.UserID)
sendMsg(arg.ChatID,arg.MsgID,'👤*¦* العضو » '..arg.UserName..' \n🎫*¦* الايدي » (`'..arg.UserID..'`)\n🛠*¦* تم طرد العضو \n✓️')
...
if redis:sismember(wsam..'banned:'..arg.ChatID,UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,'👤*¦* العضو » '..UserName..' \n🎫*¦* الايدي » (`'..UserID..'`)\n🛠*¦* تم بالتأكيد حظره \n✓️') 
end
kick_user(UserID,arg.ChatID,function(arg,data)
if data.ID == "Error" and data.code_ == 400 then
...
USERCAR = utf8.len(USERNAME)
GetChatMember(arg.ChatID,arg.UserID,function(arg,data)
if (data.status_.ID == "ChatMemberStatusKicked" or redis:sismember(wsam..'banned:'..arg.ChatID,arg.UserID)) then
...
local UserID = data.id_
UserName = Flter_Markdown(arg.UserName)
if not redis:sismember(wsam..'banned:'..arg.ChatID,UserID) then 
...
USERNAME = USERNAME:gsub([[\_]],"_")
USERCAR = utf8.len(USERNAME)
if redis:sismember(wsam..'is_silent_users:'..arg.ChatID,arg.UserID) then 
SendMention(arg.ChatID,arg.UserID,arg.MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..arg.UserID..'}\n🛠¦ تم بالتأكيد كتمه  \n✓️',12,utf8.len(USERNAME)) 
else
redis:hset(wsam..'username:'..arg.UserID,'username',USERNAME)
redis:sadd(wsam..'is_silent_users:'..arg.ChatID,arg.UserID)
SendMention(arg.ChatID,arg.UserID,arg.MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..arg.UserID..'}\n🛠¦ تم كتمه \n✓️',12,utf8.len(USERNAME)) 
end
end,{ChatID=arg.ChatID,UserID=UserID,MsgID=arg.MsgID})
end,{ChatID=msg.chat_id_,MsgID=msg.id_})
...
if redis:sismember(wsam..'is_silent_users:'..arg.ChatID,UserID) then 
sendMsg(arg.ChatID,arg.MsgID,'👤*¦* العضو » '..UserName..' \n🎫*¦* الايدي » (`'..UserID..'`)\n🛠*¦* تم بالتأكيد كتمه \n✓️') 
else
redis:hset(wsam..'username:'..UserID,'username',UserName)
redis:sadd(wsam..'is_silent_users:'..arg.ChatID,UserID)
sendMsg(arg.ChatID,arg.MsgID,'👤*¦* العضو » '..UserName..' \n🎫*¦* الايدي » (`'..UserID..'`)\n🛠*¦* تم كتمه \n✓️')
end
end,{ChatID=msg.chat_id_,MsgID=msg.id_,UserName=MsgText[2]})
elseif MsgText[2] and MsgText[2]:match('^%d+$') then 
...
USERNAME = USERNAME:gsub([[\_]],"_")
USERCAR = utf8.len(USERNAME)
if not redis:sismember(wsam..'is_silent_users:'..arg.ChatID,arg.UserID) then 
...
local UserID = data.id_
UserName = Flter_Markdown(arg.UserName)
if not redis:sismember(wsam..'is_silent_users:'..arg.ChatID,UserID) then 
...
local Admins = redis:scard(wsam..'admins:'..msg.chat_id_)
redis:del(wsam..'admins:'..msg.chat_id_)
local NumMDER = redis:scard(wsam..'owners:'..msg.chat_id_)
redis:del(wsam..'owners:'..msg.chat_id_)
local MMEZEN = redis:scard(wsam..'whitelist:'..msg.chat_id_)
redis:del(wsam..'whitelist:'..msg.chat_id_)
...
USERNAME = USERNAME:gsub([[\_]],"_")
USERCAR = utf8.len(USERNAME) 
if not redis:sismember(wsam..'owners:'..arg.ChatID,arg.UserID) and not redis:sismember(wsam..'admins:'..arg.ChatID,arg.UserID) and not redis:sismember(wsam..'whitelist:'..arg.ChatID,arg.UserID) then
...
if not msg.Director then return "📛*¦* هذا الامر يخص {المطور,المنشئ,المدير} فقط  \n🚶" end
if not redis:get(wsam..'ExCmdLink'..msg.chat_id_) then
local LinkGp = ExportLink(msg.chat_id_)
if LinkGp then
LinkGp = LinkGp.result
redis:set(wsam..'linkGroup'..msg.chat_id_,LinkGp)
redis:setex(wsam..'ExCmdLink'..msg.chat_id_,120,true)
...
local Wsam_Text = Text:match("^كلهم (.*)$"),50
local t = "\n🗣¦ "..Wsam_Text.."\n━━━━━━━━━━━━━\n"
x = 0
local list = wsam.members_
for k, v in pairs(list) do
...
if not redis:get(wsam..'linkGroup'..msg.chat_id_) then 
return "📡*¦* اوه 🙀 لا يوجد رابط ☹️\n🔖*¦*لانشاء رابط ارسل { انشاء رابط } \n📡" 
end
local GroupName = redis:get(wsam..'group:name'..msg.chat_id_)
local GroupLink = redis:get(wsam..'linkGroup'..msg.chat_id_)
return "🔖¦رابـط الـمـجـمـوعه 💯\n🌿¦ "..Flter_Markdown(GroupName).." :\n\n["..GroupLink.."]\n"
end

if MsgText[1] == "وضع رابط" then
if not msg.Director then return "📛*¦*هذا الامر يخص {المطور,المنشئ,المدير} فقط  \n🚶" end
redis:setex(wsam..'linkGroup'..msg.sender_user_id_,300,true)
return '📭¦ حسننا عزيزي  ✋🏿\n🗯¦ الان ارسل الرابط الجديد 🍃'
end

if MsgText[1] == "ضع القوانين" then
if not msg.Admin then return "📛*¦* هذا الامر يخص {الادمن,المدير,المنشئ,المطور} فقط  \n🚶" end
redis:setex(wsam..'rulse:witting'..msg.sender_user_id_,300,true)
return '📭¦ حسننا عزيزي  ✋🏿\n🗯¦ الان ارسل القوانين  للمجموعه 🍃'
end

if MsgText[1] == "القوانين" then
if not redis:get(wsam..'rulse:msg'..msg.chat_id_) then 
return "📡*¦* مرحبأ عزيري 👋🏻 القوانين كلاتي 👇🏻\n🔖¦ ممنوع نشر الروابط \n🔖¦ ممنوع التكلم او نشر صور اباحيه \n🔖¦ ممنوع  اعاده توجيه\n🔖¦ ممنوع التكلم بلطائفه \n🔖¦ الرجاء احترام المدراء والادمنيه 😅\n"
else 
return "*🔖¦القوانين :*\n"..redis:get(wsam..'rulse:msg'..msg.chat_id_) 
end 
end
...
redis:set(wsam..'flood'..msg.chat_id_,MsgText[2]) 
...
if not msg.Director then return "📛*¦* هذا الامر يخص {المطور,المنشئ,المدير} فقط  \n🚶" end
local Mn3Word = redis:scard(wsam..':Filter_Word:'..msg.chat_id_)
if Mn3Word == 0 then 
return "📡*¦* عذرا لا توجد كلمات ممنوعه ليتم حذفها ✓" 
end
redis:del(wsam..':Filter_Word:'..msg.chat_id_)
...
redis:del(wsam..'rulse:msg'..msg.chat_id_)
...
if not msg.Director then return "📛*¦* هذا الامر يخص {المطور,المنشئ,المدير} فقط  \n🚶" end
if not redis:get(wsam..'welcome:msg'..msg.chat_id_) then 
return "📡*¦* اوه ☢ هنالك خطأ 🚸\n📛¦ عذرا لا يوجد ترحيب ليتم مسحه ✓" 
end
redis:del(wsam..'welcome:msg'..msg.chat_id_)
...
local NumMnsha = redis:scard(wsam..':MONSHA_BOT:'..msg.chat_id_)
if NumMnsha ==0 then 
return "📛¦ عذرا لا يوجد منشئيين ليتم مسحهم \n!" 
end
redis:del(wsam..':MONSHA_BOT:'..msg.chat_id_)
...
local list = redis:smembers(wsam..'banned:'..msg.chat_id_)
if #list == 0 then return "*📌¦ لا يوجد مستخدمين محظورين  *" end
message = '📋*¦* قائمه الاعضاء المحظورين :\n'
for k,v in pairs(list) do
StatusLeft(msg.chat_id_,v)
end 
redis:del(wsam..'banned:'..msg.chat_id_)
return "🙋🏼‍♂️*¦* أهلا عزيزي "..msg.TheRankCmd.."   \n📛¦  تم مسح {* "..#list.." *} من المحظورين  \n✓"
end

if MsgText[2] == 'المكتومين' then
if not msg.Director then return "📛*¦* هذا الامر يخص {المطور,المنشئ,المدير} فقط  \n🚶" end
local MKTOMEN = redis:scard(wsam..'is_silent_users:'..msg.chat_id_)
if MKTOMEN ==0 then 
return "📡*¦* لا يوجد مستخدمين مكتومين في المجموعه " 
end
redis:del(wsam..'is_silent_users:'..msg.chat_id_)
...
redis:del(wsam..'linkGroup'..msg.chat_id_)
...
if not msg.Admin then return "📛*¦* هذا الامر يخص {الادمن,المدير,المنشئ,المطور} فقط  \n🚶" end
redis:setex(wsam..'name:witting'..msg.sender_user_id_,300,true)
...
redis:setex(wsam..'photo:group'..msg.chat_id_..msg.sender_user_id_,300,true)
...
if not msg.Admin then return "📛*¦* هذا الامر يخص {الادمن,المدير,المنشئ,المطور} فقط  \n🚶" end
redis:setex(wsam..'about:witting'..msg.sender_user_id_,300,true) 
...
kick_user(v.user_id_,arg.ChatID,function(arg,data)  
redis:srem(wsam..':MONSHA_BOT:'..arg.ChatID,v.user_id_)
redis:srem(wsam..'whitelist:'..arg.ChatID,v.user_id_)
redis:srem(wsam..'owners:'..arg.ChatID,v.user_id_)
redis:srem(wsam..'admins:'..arg.ChatID,v.user_id_)
end)
end
NumMem = NumMem + 1
...
redis:setex(wsam..'ExpireDate:'..msg.chat_id_, extime, true)
if not redis:get(wsam..'CheckExpire::'..msg.chat_id_) then 
redis:set(wsam..'CheckExpire::'..msg.chat_id_,true) end
sendMsg(msg.chat_id_,msg.id_,'💂🏻‍♀️¦ تم شحن الاشتراك الى `'..MsgText[2]..'` يوم   ... 👍🏿')
sendMsg(SUDO_ID,0,'💂🏻‍♀️¦ تم شحن الاشتراك الى `'..MsgText[2]..'` يوم   ... 👍🏿\n🕵🏼️‍♀️¦ في مجموعه  » »  '..redis:get(wsam..'group:name'..msg.chat_id_))
else
...
if not msg.SudoUser then return "📛*¦* هذا الامر يخص {المطور} فقط  \n🚶" end
if MsgText[2] == '1' then
redis:setex(wsam..'ExpireDate:'..msg.chat_id_, 2592000, true)
if not redis:get(wsam..'CheckExpire::'..msg.chat_id_) then 
redis:set(wsam..'CheckExpire::'..msg.chat_id_,true) 
end
sendMsg(msg.chat_id_,msg.id_,'💂🏻‍♀️¦ تم تفعيل الاشتراك   👍🏿\n📆¦  الاشتراك » `30 يوم`  *(شهر)*')
sendMsg(SUDO_ID,0,'💂🏻‍♀️¦ تم تفعيل الاشتراك  👍🏿\n📆¦  الاشتراك » `30 يوم`  *(شهر)*')
end
if MsgText[2] == '2' then
redis:setex(wsam..'ExpireDate:'..msg.chat_id_,7776000,true)
if not redis:get(wsam..'CheckExpire::'..msg.chat_id_) then 
redis:set(wsam..'CheckExpire::'..msg.chat_id_,true) 
end
...
if MsgText[2] == '3' then
redis:set(wsam..'ExpireDate:'..msg.chat_id_,true)
if not redis:get(wsam..'CheckExpire::'..msg.chat_id_) then 
redis:set(wsam..'CheckExpire::'..msg.chat_id_,true) end
...
if not msg.Admin then return "📛*¦* هذا الامر يخص {الادمن,المدير,المنشئ,المطور} فقط  \n🚶" end
local check_time = redis:ttl(wsam..'ExpireDate:'..msg.chat_id_)
...
Getrtb = 'مطور اساسي ' 
elseif redis:sismember(wsam..':SUDO_BOT:',arg.UserID) then
Getrtb = 'مطور ' 
elseif redis:sismember(wsam..':MONSHA_BOT:'..arg.ChatID,arg.UserID) then
Getrtb = 'منشئ' 
elseif redis:sismember(wsam..'owners:'..arg.ChatID,arg.UserID) then
Getrtb = 'المدير ' 
elseif redis:sismember(wsam..'admins:'..arg.ChatID,arg.UserID) then
Getrtb = 'ادمن' 
elseif redis:sismember(wsam..'whitelist:'..arg.ChatID,arg.UserID) then
Getrtb = 'مميز' 
else
Getrtb = 'عضو' 
end
...
GetUserID(msg.sender_user_id_,function(arg,data)
local msgs = (redis:get(wsam..'msgs:'..msg.sender_user_id_..':'..msg.chat_id_) or 0)
local NumGha = (redis:get(wsam..':adduser:'..msg.chat_id_..':'..msg.sender_user_id_) or 0)
local photo = (redis:get(wsam..':photo:'..msg.chat_id_..':'..msg.sender_user_id_) or 0)
local sticker = (redis:get(wsam..':sticker:'..msg.chat_id_..':'..msg.sender_user_id_) or 0)
local voice = (redis:get(wsam..':voice:'..msg.chat_id_..':'..msg.sender_user_id_) or 0)
local audio = (redis:get(wsam..':audio:'..msg.chat_id_..':'..msg.sender_user_id_) or 0)
local animation = (redis:get(wsam..':animation:'..msg.chat_id_..':'..msg.sender_user_id_) or 0)
local edited = (redis:get(wsam..':edited:'..msg.chat_id_..':'..msg.sender_user_id_) or 0)
local video = (redis:get(wsam..':video:'..msg.chat_id_..':'..msg.sender_user_id_) or 0)
...
return '🧟‍♂*¦*  عدد جهہآتگ آلمـضـآفهہ‏‏ » 【'..(redis:get(wsam..':adduser:'..msg.chat_id_..':'..msg.sender_user_id_) or 0)..'】 . \n🐾'
end
...
SUDO_USER = redis:hgetall(wsam..'username:'..SUDO_ID).username
...
if not msg.Admin then return "📛*¦* هذا الامر يخص {الادمن,المدير,المنشئ,المطور} فقط  \n🚶" end
redis:set(wsam..'welcom:witting'..msg.sender_user_id_,true) 
...
if not msg.Admin then return "📛*¦* هذا الامر يخص {الادمن,المدير,المنشئ,المطور} فقط  \n🚶" end
if redis:get(wsam..'welcome:msg'..msg.chat_id_)  then
return Flter_Markdown(redis:get(wsam..'welcome:msg'..msg.chat_id_))
else 
return "🙋🏼‍♂️*¦* أهلا عزيزي "..msg.TheRankCmd.."  \n🌿¦ نورت المجموعه \n💂🏼‍♀️" 
end 
end
...
if not msg.Director then return "📛*¦* هذا الامر يخص {المطور,المنشئ,المدير} فقط  \n🚶" end
redis:set(wsam..'idwsam:edit'..msg.sender_user_id_,true) 
...
if not msg.Director then return "📛*¦* هذا الامر يخص {المطور,المنشئ,المدير} فقط  \n🚶" end
if not redis:get(wsam..'idwsam:msg'..msg.chat_id_) then 
return "📡*¦* اوه ☢ هنالك خطأ 🚸\n📛¦ عذرا لا يوجد كليشه ايدي ليتم مسحه ✓" 
end
redis:del(wsam..'idwsam:msg'..msg.chat_id_)
...
USERNAME = USERNAME:gsub([[\_]],"_")
USERCAR = utf8.len(USERNAME)
if redis:sismember(wsam..':SUDO_BOT:',arg.UserID) then 
SendMention(arg.ChatID,arg.UserID,arg.MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..arg.UserID..'}\n🛠¦ انه بالتأكيد مطور \n✓️',12,USERCAR) 
else
SendMention(arg.ChatID,arg.UserID,arg.MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..arg.UserID..'}\n🛠¦ تم ترقيته ليصبح مطور \n✓️',12,USERCAR) 
redis:hset(wsam..'username:'..arg.UserID,'username',USERNAME)
redis:sadd(wsam..':SUDO_BOT:',arg.UserID)
end
end,{ChatID=arg.ChatID,UserID=UserID,MsgID=arg.MsgID})
end,{ChatID=msg.chat_id_,MsgID=msg.id_})
end
...
local UserID = data.id_
UserName = Flter_Markdown(arg.UserName)
if redis:sismember(wsam..':SUDO_BOT:',UserID) then 
sendMsg(arg.ChatID,arg.MsgID,'👤*¦* العضو » '..UserName..' \n🎫*¦* الايدي » (`'..UserID..'`)\n🛠*¦* انه بالتأكيد مطور \n✓') 
else
redis:hset(wsam..'username:'..UserID,'username',UserName)
redis:sadd(wsam..':SUDO_BOT:',UserID)
sendMsg(arg.ChatID,arg.MsgID,'👤*¦* العضو » '..UserName..' \n🎫*¦* الايدي » (`'..UserID..'`)\n🛠*¦* تم ترقيته ليصبح مطور \n✓')
end
end,{ChatID=msg.chat_id_,MsgID=msg.id_,UserName=MsgText[2]})
end 
...
USERNAME = USERNAME:gsub([[\_]],"_")
USERCAR = utf8.len(USERNAME)
if not redis:sismember(wsam..':SUDO_BOT:',arg.UserID) then 
SendMention(arg.ChatID,arg.UserID,arg.MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..arg.UserID..'}\n🛠¦ انه بالتأكيد ليس مطور \n✓️',12,utf8.len(USERNAME)) 
else
SendMention(arg.ChatID,arg.UserID,arg.MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..arg.UserID..'}\n🛠¦ تم تنزيله من المطورين \n✓️',12,utf8.len(USERNAME)) 
redis:srem(wsam..':SUDO_BOT:',arg.UserID)
end  
end,{ChatID=arg.ChatID,UserID=UserID,MsgID=arg.MsgID})
end,{ChatID=msg.chat_id_,MsgID=msg.id_})
end
...
local UserID = data.id_
UserName = Flter_Markdown(arg.UserName)
if not redis:sismember(wsam..':SUDO_BOT:',UserID) then 
sendMsg(arg.ChatID,arg.MsgID,'👤*¦* العضو » '..UserName..' \n🎫*¦* الايدي » (`'..UserID..'`)\n🛠*¦* انه بالتأكيد ليس مطور \n✓')
else
redis:srem(wsam..':SUDO_BOT:',UserID)
sendMsg(arg.ChatID,arg.MsgID,'👤*¦* العضو » '..UserName..' \n🎫*¦* الايدي » (`'..UserID..'`)\n🛠*¦* تم تنزيله من المطورين \n✓') 
end
end,{ChatID=msg.chat_id_,MsgID=msg.id_,UserName=MsgText[2]})
end 
...
local groups = redis:smembers(wsam..'group:ids')
...
local pv = redis:smembers(wsam..'users')
local NumPvDel = 0
for i = 1, #pv do
...
if arg.pvC == arg.i then 
if NumPvDel == 0 then
PvWsam = '👨🏼‍⚕️| جـيـد , لا يوجد مشتركين وهمي\n\n'
else
local SenderOk = arg.pvC - NumPvDel
PvWsam = '👥*¦* عدد المشتركين •⊱ { *'..arg.pvC..'* } ⊰•\n♻️*¦* تـم تنظيف  •⊱ { *'..NumPvDel..'* } ⊰• مشترك \n📊*¦* اصبح العدد الحقيقي الان •⊱ { *'..SenderOk..'* } ⊰• من المشتركين\n\n'
end
end
end,{chat_id_=msg.chat_id_,id_=msg.id_,pvC=#pv,pv=pv[i],i=i})
end
...
local GroupDel = arg.groupsC - GroupsIsFound 
if GroupDel == 0 then
sendMsg(arg.chat_id_,arg.id_,PvWsam..'💯*¦* جـيـد , لا توجد مجموعات وهميه \n✓')
else
sendMsg(arg.chat_id_,arg.id_,PvWsam..'📑*¦* عدد المجموعات •⊱ { *'..arg.groupsC..'* } ⊰•\n🚸*¦* تـم تنظيف  •⊱ { *'..GroupDel..'* } ⊰• مجموعه \n📉*¦* اصبح العدد الحقيقي الان •⊱ { *'..GroupsIsFound..'* } ⊰• مجموعه\n✓')
end
end
end,{chat_id_=msg.chat_id_,id_=msg.id_,groupsC=#groups,group=groups[i],i=i})
end
...
redis:setex(wsam..'welcom_ph:witting'..msg.sender_user_id_,300,true) 
...
local Photo_Weloame = redis:get(wsam..':WELCOME_BOT')
...
sendPhoto(msg.chat_id_,msg.id_,Photo_Weloame,[[💯¦ مـرحبآ آنآ بوت آسـمـي ]]..redis:get(wsam..':NameBot:')..[[ 🎖
...
return send_msg(msg.chat_id_,redis:get(wsam..":TEXT_SUDO") or '🗃¦ لا توجد كليشه المطور .\n📰¦ يمكنك اضافه كليشه من خلال الامر\n       " `ضع كليشه المطور` " \n📡',msg.id_)
end

if MsgText[1] == "اذاعه عام بالتوجيه" or MsgText[1] == "اذاعه عام بالتوجيه 📣" then
if not msg.SudoUser then return"📛*¦* هذا الامر يخص {المطور} فقط  \n🚶" end
if not msg.SudoBase and not redis:get(wsam..'lock_brod') then 
return "📡*¦* الاذاعه مقفوله من قبل المطور الاساسي  🚶" 
end
redis:setex(wsam..'fwd:'..msg.sender_user_id_,300, true) 
...
redis:setex(wsam..'fwd:groupswsam'..msg.sender_user_id_,300, true) 
...
redis:setex(wsam..'fwd:number'..msg.sender_user_id_,300, true) 
...
redis:setex(wsam..'fwd:all'..msg.sender_user_id_,300, true) 
...
redis:setex(wsam..'fwd:pv'..msg.sender_user_id_,300, true) 
...
redis:setex(wsam..'fwd:groups'..msg.sender_user_id_,300, true) 
...
redis:hset(wsam..'username:'..arg.UserID,'username',USERNAME)
redis:sadd(wsam..'gban_users',arg.UserID)
...
if redis:sismember(wsam..'gban_users',UserID) then 
sendMsg(arg.ChatID,arg.MsgID,'👤*¦* العضو » '..UserName..' \n🎫*¦* الايدي » (`'..UserID..'`)\n🛠*¦* تم بالتأكيد حظره عام \n✓️') 
else
redis:hset(wsam..'username:'..UserID,'username',UserName)
redis:sadd(wsam..'gban_users',UserID)
...
if GeneralBanned(arg.UserID) then 
SendMention(arg.ChatID,arg.UserID,arg.MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..arg.UserID..'}\n🛠¦ تم الغاء حظره العام \n✓️',12,utf8.len(USERNAME)) 
redis:srem(wsam..'gban_users',arg.UserID)
else
...
local UserID = data.id_
UserName = Flter_Markdown(arg.UserName)
if not redis:sismember(wsam..'gban_users',UserID) then 
sendMsg(arg.ChatID,arg.MsgID,'👤*¦* العضو » '..UserName..' \n🎫*¦* الايدي » (`'..UserID..'`)\n🛠*¦* تم بالتأكيد الغاء حظره العام \n✓️') 
else
redis:srem(wsam..'gban_users',UserID)
sendMsg(arg.ChatID,arg.MsgID,'👤*¦* العضو » '..UserName..' \n🎫*¦* الايدي » (`'..UserID..'`)\n🛠*¦*  تم الغاء حظره العام \n✓️')
end
end,{ChatID=msg.chat_id_,MsgID=msg.id_,UserName=MsgText[2]})
end 
...
if not msg.Admin then return "📛*¦* هذا الامر يخص {الادمن,المدير,المنشئ,المطور} فقط  \n🚶" end
redis:del(wsam..'welcom:witting'..msg.sender_user_id_,
wsam..'rulse:witting'..msg.sender_user_id_,
wsam..'rulse:witting'..msg.sender_user_id_,
wsam..'name:witting'..msg.sender_user_id_,
wsam..'about:witting'..msg.sender_user_id_,
wsam..'fwd:all'..msg.sender_user_id_,
wsam..'fwd:pv'..msg.sender_user_id_,
wsam..'fwd:groups'..msg.sender_user_id_,
wsam..'fwd:number'..msg.sender_user_id_,
wsam..'fwd:groupswsam'..msg.sender_user_id_,
wsam..':ForceSub:'..msg.sender_user_id_,
wsam..'namebot:witting'..msg.sender_user_id_,
wsam..'addrd_all:'..msg.sender_user_id_,
wsam..'delrd:'..msg.sender_user_id_,
wsam..'addrd:'..msg.sender_user_id_,
wsam..'delrdall:'..msg.sender_user_id_,
wsam..'text_sudo:witting'..msg.sender_user_id_,
wsam..'addrd:'..msg.chat_id_..msg.sender_user_id_,
wsam..'addrd_all:'..msg.chat_id_..msg.sender_user_id_,
wsam..'CHENG:ID'..msg.chat_id_..msg.sender_user_id_)
...
if not msg.SudoBase then return "📛*¦* هذا الامر يخص {المطور الاساسي} فقط  \n🚶" end
local GetVerison = https.request('https://raw.githubusercontent.com/wsamCli/wsambot/master/GetVersion.txt') or 0
if GetVerison > version then
UpdateSourceStart = true
sendMsg(msg.chat_id_,msg.id_,'??*¦* يوجد تحديث جديد الان \n📡*¦* جاري تنزيل وتثبيت التحديث  ...')
redis:set(wsam..":VERSION",GetVerison)
return false
else
...
local inline = {{{text="فير : فـيـر 🍃",url="t.me/wsamcli"}}}
send_key(msg.sender_user_id_,'  [قناة السورس : فـيـر](t.me/wsamcli)',nil,inline,msg.id_)
return false
end

if (MsgText[1]== "الاحصائيات 📈" or MsgText[1]=="الاحصائيات") then
if not msg.SudoBase then return"📛*¦* هذا الامر يخص {المطور الاساسي} فقط  \n🚶" end
return 'الاحصائيات : 📈 \n\n👥*¦* عدد المجموعات المفعله : '..redis:scard(wsam..'group:ids')..'\n👤*¦* عدد المشتركين في البوت : '..redis:scard(wsam..'users')..'\n📡'
end
---------------[End Function data] -----------------------
if MsgText[1]=="اضف رد عام" or MsgText[1]=="اضف رد عام ➕" then
if not msg.SudoBase then return"📛*¦* هذا الامر يخص {المطور الاساسي} فقط  \n🚶" end
redis:setex(wsam..'addrd_all:'..msg.chat_id_..msg.sender_user_id_,300,true)
redis:del(wsam..'allreplay:'..msg.chat_id_..msg.sender_user_id_)
return "📭¦ حسننا الان ارسل كلمة الرد العام 🍃\n"
end

if MsgText[1]== 'مسح' and MsgText[2]== 'الردود' then
if not msg.Director then return "📛*¦* هذا الامر يخص {المطور,المنشئ,المدير} فقط  \n🚶" end
local names 	= redis:exists(wsam..'replay:'..msg.chat_id_)
local photo 	= redis:exists(wsam..'replay_photo:group:'..msg.chat_id_)
local voice 	= redis:exists(wsam..'replay_voice:group:'..msg.chat_id_)
local imation   = redis:exists(wsam..'replay_animation:group:'..msg.chat_id_)
local audio	 	= redis:exists(wsam..'replay_audio:group:'..msg.chat_id_)
local sticker 	= redis:exists(wsam..'replay_sticker:group:'..msg.chat_id_)
local video 	= redis:exists(wsam..'replay_video:group:'..msg.chat_id_)
if names or photo or voice or imation or audio or sticker or video then
redis:del(wsam..'replay:'..msg.chat_id_,
wsam..'replay_photo:group:'..msg.chat_id_,
wsam..'replay_voice:group:'..msg.chat_id_,
wsam..'replay_animation:group:'..msg.chat_id_,
wsam..'replay_audio:group:'..msg.chat_id_,
wsam..'replay_sticker:group:'..msg.chat_id_,
wsam..'replay_video:group:'..msg.chat_id_,
wsam..':caption_replay:'..msg.chat_id_)
...
local names 	= redis:exists(wsam..'replay:all')
local photo 	= redis:exists(wsam..'replay_photo:group:')
local voice 	= redis:exists(wsam..'replay_voice:group:')
local imation 	= redis:exists(wsam..'replay_animation:group:')
local audio 	= redis:exists(wsam..'replay_audio:group:')
local sticker 	= redis:exists(wsam..'replay_sticker:group:')
local video 	= redis:exists(wsam..'replay_video:group:')
if names or photo or voice or imation or audio or sticker or video then
redis:del(wsam..'replay:all',
wsam..'replay_photo:group:',
wsam..'replay_voice:group:',
wsam..'replay_animation:group:',
wsam..'replay_audio:group:',
wsam..'replay_sticker:group:',
wsam..'replay_video:group:',
wsam..':caption_replay_All:')
...
if not msg.SudoBase then return"📛*¦* هذا الامر يخص {المطور الاساسي} فقط  \n🚶" end
redis:set(wsam..'delrdall:'..msg.sender_user_id_,true) 
return "📭¦ حسننا عزيزي  ✋🏿\n🗯¦ الان ارسل الرد لمسحها من  المجموعات 🍃"
end

if MsgText[1]== 'مسح' and MsgText[2]== 'رد' then
if not msg.Director then return "📛*¦* هذا الامر يخص {المطور,المنشئ,المدير} فقط  \n🚶" end
redis:set(wsam..'delrd:'..msg.sender_user_id_,true)
...
if not msg.Director then return "📛*¦* هذا الامر يخص {المطور,المنشئ,المدير} فقط  \n🚶" end
local names  	= redis:hkeys(wsam..'replay:'..msg.chat_id_)
local photo 	= redis:hkeys(wsam..'replay_photo:group:'..msg.chat_id_)
local voice  	= redis:hkeys(wsam..'replay_voice:group:'..msg.chat_id_)
local imation 	= redis:hkeys(wsam..'replay_animation:group:'..msg.chat_id_)
local audio 	= redis:hkeys(wsam..'replay_audio:group:'..msg.chat_id_)
local sticker 	= redis:hkeys(wsam..'replay_sticker:group:'..msg.chat_id_)
local video 	= redis:hkeys(wsam..'replay_video:group:'..msg.chat_id_)
...
local names 	= redis:hkeys(wsam..'replay:all')
local photo 	= redis:hkeys(wsam..'replay_photo:group:')
local voice 	= redis:hkeys(wsam..'replay_voice:group:')
local imation 	= redis:hkeys(wsam..'replay_animation:group:')
local audio 	= redis:hkeys(wsam..'replay_audio:group:')
local sticker 	= redis:hkeys(wsam..'replay_sticker:group:')
local video 	= redis:hkeys(wsam..'replay_video:group:')
if #names==0 and #photo==0 and #voice==0 and #imation==0 and #audio==0 and #sticker==0 and #video==0 then 
return '🚸*¦* لا يوجد ردود مضافه حاليا \n❕' 
end
local ii = 1
...
if not msg.Director then return "📛*¦* هذا الامر يخص {المطور,المنشئ,المدير} فقط  \n🚶" end
redis:setex(wsam..'addrd:'..msg.chat_id_..msg.sender_user_id_,300,true) 
redis:del(wsam..'replay1'..msg.chat_id_..msg.sender_user_id_)
return "📭¦ حسننا , الان ارسل كلمه الرد \n-"
end

if MsgText[1] == "ضع اسم للبوت" or MsgText[1]== 'ضع اسم للبوت ©' then
if not msg.SudoBase then return"📛*¦* هذا الامر يخص {المطور الاساسي} فقط  \n🚶" end
redis:setex(wsam..'namebot:witting'..msg.sender_user_id_,300,true)
...
ISP=`curl https://wsam.saied.us/ISP.php`
...
if not msg.Admin then return "📛*¦* هذا الامر يخص {الادمن,المدير,المنشئ,المطور} فقط  \n🚶" end
if not redis:get(wsam..'lave_me'..msg.chat_id_) then 
return "🙋🏼‍♂️*¦* أهلا عزيزي "..msg.TheRankCmd.."\n📡*¦* المغادره بالتاكيد تم تفعيلها\n✓" 
else 
redis:del(wsam..'lave_me'..msg.chat_id_) 
return "🙋🏼‍♂️*¦* أهلا عزيزي "..msg.TheRankCmd.."\n📡*¦* تم تفعيل المغادره \n✓" 
end 
end
if MsgText[1] == "تعطيل" and MsgText[2] == "اطردني" then
if not msg.Admin then return "📛*¦* هذا الامر يخص {الادمن,المدير,المنشئ,المطور} فقط  \n🚶" end
if redis:get(wsam..'lave_me'..msg.chat_id_) then 
return "🙋🏼‍♂️*¦* أهلا عزيزي "..msg.TheRankCmd.."\n📡*¦* المغادره من قبل البوت بالتأكيد معطله\n✓" 
else
redis:set(wsam..'lave_me'..msg.chat_id_,true)  
...
💭┇ قناة السورس ☜ @abuwsamksa 
]]
end

if MsgText[1] == "متجر الملفات" or MsgText[1]:lower() == "/store"  then
if not msg.SudoBase then return"📛*¦* هذا الامر يخص {المطور الاساسي} فقط  \n🚶" end
local Get_Files, res = https.request("https://raw.githubusercontent.com/abuwsamhak/WSAM11BOT/master/GetFiles.json")
if res == 200 then
...
local Get_Files, res = https.request("https://raw.githubusercontent.com/wsamCli/wsambot/master/plugins/"..FileName)
if res == 200 then
...
if not msg.SudoBase then return"📛*¦* هذا الامر يخص {المطور الاساسي} فقط  \n🚶" end
if redis:get(wsam..":UserNameChaneel") then
return "📑| اهلا عزيزي المطور \n🔖| الاشتراك بالتأكيد مفعل"
else
redis:setex(wsam..":ForceSub:"..msg.sender_user_id_,350,true)
...
if not msg.SudoBase then return"📛*¦* هذا الامر يخص {المطور الاساسي} فقط  \n🚶" end
local SubDel = redis:del(wsam..":UserNameChaneel")
if SubDel == 1 then
...
if not msg.SudoBase then return"📛*¦* هذا الامر يخص {المطور الاساسي} فقط  \n🚶" end
local UserChaneel = redis:get(wsam..":UserNameChaneel")
if UserChaneel then
...
local function dWsam(msg)

if msg.type == "pv" then 

if not msg.SudoUser then
local msg_pv = tonumber(redis:get(wsam..'user:'..msg.sender_user_id_..':msgs') or 0)
if msg_pv > 5 then
redis:setex(wsam..':mute_pv:'..msg.sender_user_id_,18000,true)   
return sendMsg(msg.chat_id_,0,'*📛¦* تم حظرك من البوت بسبب التكرار \n🛠') 
end
redis:setex(wsam..'user:'..msg.sender_user_id_..':msgs',2,msg_pv+1)
end

if msg.text=="الاذاعة والاشتراك الاجباري 🔊" and msg.SudoBase then
local text = '🎖¦ آهہ‏‏لآ عزيزي آلمـطـور 🍃\n💰¦ اليک قائمهہ‏‏ آلاذاعهہ‏‏ وآلاشتراك الاجباري 🛠\n...\n\n🚸¦ تسـتطـيع‏‏ آلتحگم بگل آلآوآمـر آلمـمـوجودهہ‏‏ بآلگيبورد\n⚖️¦ فقط آضـغط ع آلآمـر آلذي تريد تنفيذهہ‏‏'
local keyboard = {
...
if msg.text=="/start" or msg.text=="رجوع 🔙" then
...
{"الاذاعة والاشتراك الاجباري 🔊"},
 {"ايدي 🆔","قناة السورس 📡"},
  {"السيرفر 📟","نسخه احتياطيه للمجموعات 📦"},
 {"إعادة تشغيل 🔁","تحديث السورس ™"},
 {"الغاء الامر ✖️"}}
return send_key(msg.sender_user_id_,text,keyboard,nil,msg.id_)
else
redis:sadd(wsam..'users',msg.sender_user_id_)
SUDO_USER = redis:hgetall(wsam..'username:'..SUDO_ID).username
if SUDO_USER:match('@[%a%d_]+') then 
...
if redis:get(wsam..'lock_service') then 
text = [[💯¦ مـرحبآ آنآ بوت آسـمـي []]..redis:get(wsam..':NameBot:')..[[] 🎖
💰¦ آختصـآصـي حمـآيهہ‏‏ آلمـجمـوعآت
📛¦ مـن آلسـبآم وآلتوجيهہ‏‏ وآلتگرآر وآلخ...
...
local MSG_ID = (redis:get(wsam.."USER_MSG_TWASEL"..data.forward_info_.date_) or 1)
msg = arg.msg
local SendOk = false
...
if not redis:get(wsam..'lock_twasel') then
...
redis:setex(wsam.."USER_MSG_TWASEL"..msg.date_,43200,msg.id_)
...
--====================== Reply Only Group =====================================
if msg.Admin and redis:get(wsam..'addrd:'..msg.chat_id_..msg.sender_user_id_) and redis:get(wsam..'replay1'..msg.chat_id_..msg.sender_user_id_) then
local klma = redis:get(wsam..'replay1'..msg.chat_id_..msg.sender_user_id_)
if msg.content_ and msg.content_.caption_ then redis:hset(wsam..':caption_replay:'..msg.chat_id_,klma,msg.content_.caption_) end
if msg.text then 
redis:del(wsam..'addrd:'..msg.chat_id_..msg.sender_user_id_)
...
redis:hset(wsam..'replay:'..msg.chat_id_,klma,Flter_Markdown(msg.text))
...
redis:hset(wsam..'replay_photo:group:'..msg.chat_id_,klma,photo_id)
redis:del(wsam..'addrd:'..msg.chat_id_..msg.sender_user_id_)
return sendMsg(msg.chat_id_,msg.id_,'🗂¦ تم اضافه صوره للرد بنجاح ✓\n🗂¦ يمكنك ارسال (['..klma..']) لاضهار الصوره الاتيه .')
elseif msg.content_.ID == "MessageVoice" then
redis:hset(wsam..'replay_voice:group:'..msg.chat_id_,klma,msg.content_.voice_.voice_.persistent_id_)
redis:del(wsam..'addrd:'..msg.chat_id_..msg.sender_user_id_)
return sendMsg(msg.chat_id_,msg.id_,'🗂¦ تم اضافه بصمه صوت للرد بنجاح ✓\n🗂¦ يمكنك ارسال (['..klma..']) لسماع البصمه الاتيه .')
elseif msg.content_.ID == "MessageAnimation" then
redis:hset(wsam..'replay_animation:group:'..msg.chat_id_,klma,msg.content_.animation_.animation_.persistent_id_)
redis:del(wsam..'addrd:'..msg.chat_id_..msg.sender_user_id_)
return sendMsg(msg.chat_id_,msg.id_,'🗂¦ تم اضافه متحركه للرد بنجاح ✓\n🗂¦ يمكنك ارسال (['..klma..']) لاضهار الصوره الاتيه .')
elseif msg.content_.ID == "MessageVideo" then
redis:hset(wsam..'replay_video:group:'..msg.chat_id_,klma,msg.content_.video_.video_.persistent_id_)
redis:del(wsam..'addrd:'..msg.chat_id_..msg.sender_user_id_)
return sendMsg(msg.chat_id_,msg.id_,'🗂¦ تم اضافه فيديو للرد بنجاح ✓\n🗂¦ يمكنك ارسال (['..klma..']) لاضهار الفيديو الاتي .')
elseif msg.content_.ID == "MessageAudio" then
redis:hset(wsam..'replay_audio:group:'..msg.chat_id_,klma,msg.content_.audio_.audio_.persistent_id_)
redis:del(wsam..'addrd:'..msg.chat_id_..msg.sender_user_id_)
return sendMsg(msg.chat_id_,msg.id_,'🗂¦ تم اضافه للصوت للرد بنجاح ✓\n🗂¦ يمكنك ارسال (['..klma..']) لاضهار الصوت الاتي .')
elseif msg.content_.ID == "MessageSticker" then
redis:hset(wsam..'replay_sticker:group:'..msg.chat_id_,klma,msg.content_.sticker_.sticker_.persistent_id_)
redis:del(wsam..'addrd:'..msg.chat_id_..msg.sender_user_id_)
...
--====================== Reply All Groups =====================================
if msg.SudoUser and redis:get(wsam..'addrd_all:'..msg.chat_id_..msg.sender_user_id_) and redis:get(wsam..'allreplay:'..msg.chat_id_..msg.sender_user_id_) then
local klma = redis:get(wsam..'allreplay:'..msg.chat_id_..msg.sender_user_id_)
if msg.content_.caption_ then redis:hset(wsam..':caption_replay_All:',klma,msg.content_.caption_) end
if msg.text then
redis:del(wsam..'addrd_all:'..msg.chat_id_..msg.sender_user_id_)
if utf8.len(msg.text) > 550 then 
sendMsg(msg.chat_id_,msg.id_,"📛¦ عذرا غير مسموح باضافه جواب الرد باكثر من 550 حرف تم الغاء الامر\n❕")
return false
end
redis:hset(wsam..'replay:all',klma,Flter_Markdown(msg.text))
...
redis:hset(wsam..'replay_photo:group:',klma,photo_id)
redis:del(wsam..'addrd_all:'..msg.chat_id_..msg.sender_user_id_)
return sendMsg(msg.chat_id_,msg.id_,'🗂¦ تم اضافه صوره للرد العام ✓\n🗂¦ يمكنك ارسال (['..klma..']) لاضهار الصوره الاتيه .')
elseif msg.content_.ID == "MessageVoice" then
redis:hset(wsam..'replay_voice:group:',klma,msg.content_.voice_.voice_.persistent_id_)
redis:del(wsam..'addrd_all:'..msg.chat_id_..msg.sender_user_id_)
return sendMsg(msg.chat_id_,msg.id_,'🗂¦ تم اضافه بصمه صوت للرد العام ✓\n🗂¦ يمكنك ارسال (['..klma..']) لسماع البصمه الاتيه .')
elseif msg.content_.ID == "MessageAnimation" then
redis:hset(wsam..'replay_animation:group:',klma,msg.content_.animation_.animation_.persistent_id_)
redis:del(wsam..'addrd_all:'..msg.chat_id_..msg.sender_user_id_)
return sendMsg(msg.chat_id_,msg.id_,'🗂¦ تم اضافه متحركه للرد العام ✓\n🗂¦ يمكنك ارسال (['..klma..']) لاضهار الصوره الاتيه .')
elseif msg.content_.ID == "MessageVideo" then
redis:hset(wsam..'replay_video:group:',klma,msg.content_.video_.video_.persistent_id_)
redis:del(wsam..'addrd_all:'..msg.chat_id_..msg.sender_user_id_)
return sendMsg(msg.chat_id_,msg.id_,'🗂¦ تم اضافه فيديو للرد العام ✓\n🗂¦ يمكنك ارسال (['..klma..']) لاضهار الفيديو الاتي .')
elseif msg.content_.ID == "MessageAudio" then
redis:hset(wsam..'replay_audio:group:',klma,msg.content_.audio_.audio_.persistent_id_)
redis:del(wsam..'addrd_all:'..msg.chat_id_..msg.sender_user_id_)
return sendMsg(msg.chat_id_,msg.id_,'🗂¦ تم اضافه للصوت للرد العام ✓\n🗂¦ يمكنك ارسال (['..klma..']) لاضهار الصوت الاتي .')
elseif msg.content_.ID == "MessageSticker" then
redis:hset(wsam..'replay_sticker:group:',klma,msg.content_.sticker_.sticker_.persistent_id_)
redis:del(wsam..'addrd_all:'..msg.chat_id_..msg.sender_user_id_)
...
if msg.SudoBase and redis:get(wsam..":ForceSub:"..msg.sender_user_id_) then
if msg.text:match("^@[%a%d_]+$") then
redis:del(wsam..":ForceSub:"..msg.sender_user_id_)
...
redis:set(wsam..":UserNameChaneel",msg.text)
...
if msg.SudoBase and redis:get(wsam..'namebot:witting'..msg.sender_user_id_) then --- استقبال اسم البوت 
if utf8.len(msg.text) >= 50 then 
sendMsg(msg.chat_id_,msg.id_,"📛¦ لا يمكنك وضع اسم باكثر من 50 حرف\n❕")
else
redis:del(wsam..'namebot:witting'..msg.sender_user_id_)
redis:set(wsam..':NameBot:',msg.text)
Start_Bot() 
...
if msg.SudoBase and redis:get(wsam..'addrd_all:'..msg.chat_id_..msg.sender_user_id_) and not redis:get(wsam..'allreplay:'..msg.chat_id_..msg.sender_user_id_) then -- استقبال الرد لكل المجموعات
if utf8.len(msg.text) > 25 then 
sendMsg(msg.chat_id_,msg.id_,"📛¦ عذرا غير مسموح باضافه كلمه الرد باكثر من 25 حرف \n❕")
else
redis:hdel(wsam..'replay_photo:group:',msg.text)
redis:hdel(wsam..'replay_voice:group:',msg.text)
redis:hdel(wsam..'replay_animation:group:',msg.text)
redis:hdel(wsam..'replay_audio:group:',msg.text)
redis:hdel(wsam..'replay_sticker:group:',msg.text)
redis:hdel(wsam..'replay_video:group:',msg.text)
redis:setex(wsam..'allreplay:'..msg.chat_id_..msg.sender_user_id_,300,msg.text)
sendMsg(msg.chat_id_,msg.id_,"📜¦ جيد , يمكنك الان ارسال جوا ب الردالعام \n🔛¦ [[ نص,صوره,فيديو,متحركه,بصمه,اغنيه ]] ✓\n-")
end
return false
end

if msg.SudoUser and redis:get(wsam..'delrdall:'..msg.sender_user_id_) then
redis:del(wsam..'delrdall:'..msg.sender_user_id_)
local names = redis:hget(wsam..'replay:all',msg.text)
local photo =redis:hget(wsam..'replay_photo:group:',msg.text)
local voice = redis:hget(wsam..'replay_voice:group:',msg.text)
local animation = redis:hget(wsam..'replay_animation:group:',msg.text)
local audio = redis:hget(wsam..'replay_audio:group:',msg.text)
local sticker = redis:hget(wsam..'replay_sticker:group:',msg.text)
local video = redis:hget(wsam..'replay_video:group:',msg.text)
if not (names or photo or voice or animation or audio or sticker or video) then
sendMsg(msg.chat_id_,msg.id_,'💬*¦* هذا الرد ليس مضاف في قائمه الردود 📛')
else
redis:hdel(wsam..'replay:all',msg.text)
redis:hdel(wsam..'replay_photo:group:',msg.text)
redis:hdel(wsam..'replay_voice:group:',msg.text)
redis:hdel(wsam..'replay_audio:group:',msg.text)
redis:hdel(wsam..'replay_animation:group:',msg.text)
redis:hdel(wsam..'replay_sticker:group:',msg.text)
redis:hdel(wsam..'replay_video:group:',msg.text)
redis:hdel(wsam..':caption_replay_All:',msg.text)
...
if msg.SudoBase and redis:get(wsam..'text_sudo:witting'..msg.sender_user_id_) then -- استقبال كليشه المطور
if utf8.len(msg.text) > 2500 then 
sendMsg(msg.chat_id_,msg.id_,"📛¦ لا يمكنك وضع كليشه باكثر من 2500 حرف\n❕")
else
redis:del(wsam..'text_sudo:witting'..msg.sender_user_id_) 
redis:set(wsam..':TEXT_SUDO',msg.text)
send_msg(msg.chat_id_, "📜*¦* تم وضع الكليشه بنجاح كلاتي 👋🏻\n\n*{*  "..msg.text.."  *}*\n✓",msg.id_)
end
return false
end

if msg.Admin and redis:get(wsam..'welcom:witting'..msg.sender_user_id_) then -- استقبال كليشه الترحيب
if utf8.len(msg.text) > 2500 then 
sendMsg(msg.chat_id_,msg.id_,"📛¦ لا يمكنك وضع كليشه باكثر من 2500 حرف\n❕")
else
redis:del(wsam..'welcom:witting'..msg.sender_user_id_) 
redis:set(wsam..'welcome:msg'..msg.chat_id_,msg.text)
sendMsg(msg.chat_id_,msg.id_,"📜*¦* تم وضع الترحيب بنجاح  👋\n✓" )
end
return false
end

if msg.Admin and redis:get(wsam..'idwsam:edit'..msg.sender_user_id_) then -- استقبال كليشه الايدي
if utf8.len(msg.text) > 1500 then 
sendMsg(msg.chat_id_,msg.id_,"📛¦ لا يمكنك وضع كليشه باكثر من 1500 حرف\n❕")
else
redis:del(wsam..'idwsam:edit'..msg.sender_user_id_) 
redis:set(wsam..'idwsam:msg'..msg.chat_id_,msg.text)
sendMsg(msg.chat_id_,msg.id_,"📜*¦* تم وضع كليشه الايدي بنجاح  👋\n✓" )
end
return false
end

if msg.Admin and redis:get(wsam..'rulse:witting'..msg.sender_user_id_) then --- استقبال القوانين
if utf8.len(msg.text) > 2500 then 
sendMsg(msg.chat_id_,msg.id_,"📛¦ لا يمكنك وضع قوانين باكثر من 2500 حرف\n❕")
else
redis:del(wsam..'rulse:witting'..msg.sender_user_id_) 
redis:set(wsam..'rulse:msg'..msg.chat_id_,Flter_Markdown(msg.text)) 
sendMsg(msg.chat_id_,msg.id_,'📜*¦* مرحبآ عزيزي\n📦¦ تم حفظ القوانين بنجاح ✓\n🔖¦ ارسل [[ القوانين ]] لعرضها \n💬✓')
end
return false
end

if msg.Admin and redis:get(wsam..'name:witting'..msg.sender_user_id_) then --- استقبال الاسم
if utf8.len(msg.text) > 550 then 
sendMsg(msg.chat_id_,msg.id_,"📛¦ لا يمكنك اسم المجموعه باكثر من 550 حرف \n❕")
else
redis:del(wsam..'name:witting'..msg.sender_user_id_) 
tdcli_function({ID="ChangeChatTitle",chat_id_=msg.chat_id_,title_=msg.text},dl_cb,nil)
end
return false
end

if msg.Admin and redis:get(wsam..'about:witting'..msg.sender_user_id_) then --- استقبال الوصف
if utf8.len(msg.text) > 1000 then 
sendMsg(msg.chat_id_,msg.id_,"📛¦ لا يمكنك وضع وصف باكثر من 1000 حرف \n❕")
else
redis:del(wsam..'about:witting'..msg.sender_user_id_) 
...
if redis:get(wsam..'linkGroup'..msg.sender_user_id_,link) then --- استقبال الرابط
redis:del(wsam..'linkGroup'..msg.sender_user_id_,link) 
redis:set(wsam..'linkGroup'..msg.chat_id_,Flter_Markdown(msg.text)) 
return sendMsg(msg.chat_id_,msg.id_,'🗯¦ الان تم وضع الرابط جديد 🍃')
end

if redis:get(wsam..'fwd:all'..msg.sender_user_id_) then ---- استقبال رساله الاذاعه عام
sendMsg(msg.chat_id_,msg.id_,'??| اهلا عزيزي المطور \n🔖| جاري نشر التوجيه للمجموعات وللمشتركين ...')			
redis:del(wsam..'fwd:all'..msg.sender_user_id_)
local pv = redis:smembers(wsam..'users')  
local groups = redis:smembers(wsam..'group:ids')
...
if redis:get(wsam..'fwd:pv'..msg.sender_user_id_) then ---- استقبال رساله الاذاعه خاص
sendMsg(msg.chat_id_,msg.id_,'📑| اهلا عزيزي المطور \n🔖| جاري نشر الرساله للمشتركين ...')			
redis:del(wsam..'fwd:pv'..msg.sender_user_id_)
local pv = redis:smembers(wsam..'users')
if #pv >= 300 then
end
local NumPvDel = 0
...
if redis:get(wsam..'fwd:groups'..msg.sender_user_id_) then ---- استقبال رساله الاذاعه خاص
sendMsg(msg.chat_id_,msg.id_,'📑| اهلا عزيزي المطور \n🔖| جاري نشر الرساله للمجموعات ...')			
redis:del(wsam..'fwd:groups'..msg.sender_user_id_)
local groups = redis:smembers(wsam..'group:ids')
if #groups >= 300 then
end
...
if msg.forward_info_ and redis:get(wsam..'fwd:'..msg.sender_user_id_) then
sendMsg(msg.chat_id_,msg.id_,'📑| اهلا عزيزي المطور \n🔖| جاري نشر التوجيه للمجموعات وللمشتركين ...')			
redis:del(wsam..'fwd:'..msg.sender_user_id_)
local pv = redis:smembers(wsam..'users')
local groups = redis:smembers(wsam..'group:ids')
local allgp =  #pv + #groups
if allgp == 500 then
...
if msg.forward_info_ and redis:get(wsam..'fwd:groupswsam'..msg.sender_user_id_) then
sendMsg(msg.chat_id_,msg.id_,'📑| اهلا عزيزي المطور \n🔖| جاري نشر التوجيه للمجموعات ...')			
redis:del(wsam..'fwd:groupswsam'..msg.sender_user_id_)
local groups = redis:smembers(wsam..'group:ids')
...
return sendMsg(msg.chat_id_,msg.id_,'📜*¦* تم اذاعه التوجيه بنجاح 🏌🏻\n🗣*¦* للمـجمـوعآت » *'..#groups..'*\n✓')			
 ...
 if msg.forward_info_ and redis:get(wsam..'fwd:number'..msg.sender_user_id_) then
sendMsg(msg.chat_id_,msg.id_,'📑| اهلا عزيزي المطور \n🔖| جاري نشر التوجيه للمشتركين ...')   
redis:del(wsam..'fwd:number'..msg.sender_user_id_)
local pv = redis:smembers(wsam..'users')
local allpvfwd =  #pv
...
if msg.SudoBase and  msg.content_.ID == "MessagePhoto" and redis:get(wsam..'welcom_ph:witting'..msg.sender_user_id_) then
redis:del(wsam..'welcom_ph:witting'..msg.sender_user_id_)
if msg.content_.photo_.sizes_[3] then 
photo_id = msg.content_.photo_.sizes_[3].photo_.persistent_id_
else 
photo_id = msg.content_.photo_.sizes_[0].photo_.persistent_id_
end
redis:set(wsam..':WELCOME_BOT',photo_id)
...
if msg.Admin and msg.content_.ID == "MessagePhoto" and redis:get(wsam..'photo:group'..msg.chat_id_..msg.sender_user_id_)then
redis:del(wsam..'photo:group'..msg.chat_id_..msg.sender_user_id_)
if msg.content_.photo_.sizes_[3] then 
photo_id = msg.content_.photo_.sizes_[3].photo_.persistent_id_
else 
photo_id = msg.content_.photo_.sizes_[0].photo_.persistent_id_
...
if redis:get(wsam..'addrd:'..msg.chat_id_..msg.sender_user_id_) and not redis:get(wsam..'replay1'..msg.chat_id_..msg.sender_user_id_) then 
...
redis:hdel(wsam..'replay:'..msg.chat_id_,msg.text)
redis:hdel(wsam..'replay_photo:group:'..msg.chat_id_,msg.text)
redis:hdel(wsam..'replay_voice:group:'..msg.chat_id_,msg.text)
redis:hdel(wsam..'replay_animation:group:'..msg.chat_id_,msg.text)
redis:hdel(wsam..'replay_audio:group:'..msg.chat_id_,msg.text)
redis:hdel(wsam..'replay_sticker:group:'..msg.chat_id_,msg.text)
redis:hdel(wsam..'replay_video:group:'..msg.chat_id_,msg.text)
redis:setex(wsam..'replay1'..msg.chat_id_..msg.sender_user_id_,300,msg.text)
sendMsg(msg.chat_id_,msg.id_,"📜¦ جيد , يمكنك الان ارسال جواب الرد \n🔛¦ {نص,صوره,فيديو,متحركه,بصمه,اغنيه } ✓")
return false
end

if redis:get(wsam..'delrd:'..msg.sender_user_id_) then  -- Del Replay For Group
redis:del(wsam..'delrd:'..msg.sender_user_id_)
local names 	= redis:hget(wsam..'replay:'..msg.chat_id_,msg.text)
local photo 	= redis:hget(wsam..'replay_photo:group:'..msg.chat_id_,msg.text)
local voice 	= redis:hget(wsam..'replay_voice:group:'..msg.chat_id_,msg.text)
local animation = redis:hget(wsam..'replay_animation:group:'..msg.chat_id_,msg.text)
local audio 	= redis:hget(wsam..'replay_audio:group:'..msg.chat_id_,msg.text)
local sticker 	= redis:hget(wsam..'replay_sticker:group:'..msg.chat_id_,msg.text)
local video 	= redis:hget(wsam..'replay_video:group:'..msg.chat_id_,msg.text)
if not (names or photo or voice or animation or audio or sticker or video) then
sendMsg(msg.chat_id_,msg.id_,'💬*¦* هذا الرد ليس مضاف في قائمه الردود 📛')
else
redis:hdel(wsam..'replay:'..msg.chat_id_,msg.text)
...
redis:hdel(wsam..'replay_sticker:group:'..msg.chat_id_,msg.text)
redis:hdel(wsam..'replay_video:group:'..msg.chat_id_,msg.text)
redis:hdel(wsam..':caption_replay:'..msg.chat_id_,msg.text)
...
print(" -- pinned -- ")
local msg_pin_id = redis:get(wsam..":MsgIDPin:"..msg.chat_id_)
if not msg.Director and redis:get(wsam..'lock_pin'..msg.chat_id_) then
if msg_pin_id then
...
redis:set(wsam..":MsgIDPin:"..msg.chat_id_,msg.id_)
end
...
GetUserID(msg.sender_user_id_,function(arg,data)
redis:set(wsam..'group:name'..arg.chat_id_,arg.title_)
if data.username_ then UserName = "@"..data.username_ else UserName = "احد المشرفين" end
...
if msg.content_.ID == "MessageChatAddMembers" and redis:get(wsam..'welcome:get'..msg.chat_id_) then
local adduserx = tonumber(redis:get(wsam..'user:'..msg.sender_user_id_..':msgs') or 0)
if adduserx > 3 then 
redis:del(wsam..'welcome:get'..msg.chat_id_)
end
redis:setex(wsam..'user:'..msg.sender_user_id_..':msgs',3,adduserx+1)
end

if (msg.content_.ID == "MessageChatAddMembers" or msg.content_.ID == "MessageChatJoinByLink") then
if redis:get(wsam..'welcome:get'..msg.chat_id_) then
if msg.adduserType then
welcome = (redis:get(wsam..'welcome:msg'..msg.chat_id_) or "🔖¦ مرحباً عزيزي\n🔖¦ نورت المجموعة \n💂🏼‍♀️")
welcome = welcome:gsub("{القوانين}", redis:get(wsam..'rulse:msg'..msg.chat_id_) or "🔖¦ مرحبأ عزيري 👋🏻 القوانين كلاتي 👇🏻\n🔖¦ ممنوع نشر الروابط \n🔖¦ ممنوع التكلم او نشر صور اباحيه \n🔖¦ ممنوع  اعاده توجيه \n🔖¦ ممنوع التكلم بلطائفه \n🔖¦ الرجاء احترام المدراء والادمنيه 😅\n")
if msg.addusername then UserName = '@'..msg.addusername else UserName = '< لا يوجد معرف >' end
welcome = welcome:gsub("{المجموعه}",Flter_Markdown((redis:get(wsam..'group:name'..msg.chat_id_) or '')))
...
GetUserID(msg.sender_user_id_,function(arg,data)
welcome = (redis:get(wsam..'welcome:msg'..arg.chat_id_) or "🔖¦ مرحباً عزيزي\n🔖¦ نورت المجموعة \n💂🏼‍♀️")
welcome = welcome:gsub("{القوانين}", redis:get(wsam..'rulse:msg'..arg.chat_id_) or "🔖¦ مرحبأ عزيري 👋🏻 القوانين كلاتي 👇🏻\n🔖¦ ممنوع نشر الروابط \n🔖¦ ممنوع التكلم او نشر صور اباحيه \n🔖¦ ممنوع  اعاده توجيه \n🔖¦ ممنوع التكلم بلطائفه \n🔖¦ الرجاء احترام المدراء والادمنيه 😅\n")
if data.username_ then UserName = '@'..data.username_ else UserName = '< لا يوجد معرف >' end
welcome = welcome:gsub("{المجموعه}",Flter_Markdown((redis:get(wsam..'group:name'..arg.chat_id_) or '')))
local welcome = welcome:gsub("{المعرف}",UserName)
...
if not msg.forward_info_ and msg.content_.ID ~= "MessagePhoto" and redis:get(wsam..'lock_flood'..msg.chat_id_)  then
local msgs = (redis:get(wsam..'user:'..msg.sender_user_id_..':msgs') or 0)
local NUM_MSG_MAX = (redis:get(wsam..'num_msg_max'..msg.chat_id_) or 5)
if tonumber(msgs) > tonumber(NUM_MSG_MAX) then 
redis:setex(wsam..'sender:'..msg.sender_user_id_..':'..msg.chat_id_..'flood',30,true)
GetUserID(msg.sender_user_id_,function(arg,data)
...
redis:setex(wsam..'user:'..msg.sender_user_id_..':msgs',2,msgs+1)
end
 
 
if msg.forward_info_ then
if redis:get(wsam..'mute_forward'..msg.chat_id_) then -- قفل التوجيه
...
if redis:get(wsam..'lock_woring'..msg.chat_id_) and not redis:get(wsam..':User_Fwd_Msg:'..msg.sender_user_id_..':flood') then
...
redis:setex(wsam..':User_Fwd_Msg:'..arg.sender_user_id_..':flood',15,true)
...
return false
elseif redis:get(wsam..':tqeed_fwd:'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
elseif msg.edited and msg.content_.ID ~= "MessageText" and redis:get(wsam..'lock_edit'..msg.chat_id_) then -- قفل التعديل
Del_msg(msg.chat_id_,msg.id_,function(arg,data) 
print("\27[1;31m Msg Del becuse send Edit \27[0m")
if data.ID == "Error" and data.code_ == 6 then
return sendMsg(msg.chat_id_,msg.id_,'📛*¦* لا يمكنني مسح الرساله المخالفه .\n🎟*¦* لست مشرف او ليس لدي صلاحيه  الحذف \n ❕')    
end
if redis:get(wsam..'lock_woring'..msg.chat_id_) then
GetUserID(arg.sender_user_id_,function(arg,data)
if data.username_ then USERNAME = '@'..data.username_ else USERNAME = FlterName(data) end
...
return false
elseif tonumber(msg.via_bot_user_id_) ~= 0 and redis:get(wsam..'mute_inline'..msg.chat_id_) then -- قفل الانلاين
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
if utf8.len(msg.text) > 500 and redis:get(wsam..'lock_spam'..msg.chat_id_) then -- قفل الكليشه 
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
if redis:get(wsam..'lock_woring'..arg.chat_id_) then
GetUserID(arg.sender_user_id_,function(arg,data)
if data.username_ then USERNAME = '@'..data.username_ else USERNAME = FlterName(data) end
...
and redis:get(wsam..':tqeed_link:'..msg.chat_id_)  then
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
and redis:get(wsam..'lock_link'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
return false
elseif (msg.text:match("[Hh][Tt][Tt][Pp][Ss]://") or msg.text:match("[Hh][Tt][Tt][Pp]://") or msg.text:match("[Ww][Ww][Ww].") or msg.text:match(".[Cc][Oo][Mm]") or msg.text:match(".[Tt][Kk]") or msg.text:match(".[Mm][Ll]") or msg.text:match(".[Oo][Rr][Gg]")) and redis:get(wsam..'lock_webpage'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
return false
elseif msg.text:match("#[%a%d_]+") and redis:get(wsam..'lock_tag'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
return false
elseif msg.text:match("@[%a%d_]+")  and redis:get(wsam..'lock_username'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
return false
elseif not msg.textEntityTypeBold and (msg.textEntityTypeBold or msg.textEntityTypeItalic) and redis:get(wsam..'lock_markdown'..msg.chat_id_) then 
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
return false
elseif msg.textEntityTypeTextUrl and redis:get(wsam..'lock_webpage'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
elseif msg.content_.ID == "MessageUnsupported" and redis:get(wsam..'mute_video'..msg.chat_id_) then -- قفل الفيديو
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
return false
elseif msg.content_.ID == "MessagePhoto" then
if redis:get(wsam..'mute_photo'..msg.chat_id_)  then -- قفل الصور
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
return false
elseif redis:get(wsam..':tqeed_photo:'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
elseif msg.content_.ID == "MessageVideo" then
if redis:get(wsam..'mute_video'..msg.chat_id_) then -- قفل الفيديو
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
return false
elseif redis:get(wsam..':tqeed_video:'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
elseif msg.content_.ID == "MessageDocument" and redis:get(wsam..'mute_document'..msg.chat_id_) then -- قفل الملفات
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
return false
elseif msg.content_.ID == "MessageSticker" and redis:get(wsam..'mute_sticker'..msg.chat_id_) then --قفل الملصقات
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
return false
elseif msg.content_.ID == "MessageAnimation" then
if redis:get(wsam..'mute_gif'..msg.chat_id_) then -- قفل المتحركه
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
return false
elseif redis:get(wsam..':tqeed_gif:'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
elseif msg.content_.ID == "MessageContact" and redis:get(wsam..'mute_contact'..msg.chat_id_) then -- قفل الجهات
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
return false
elseif msg.content_.ID == "MessageLocation" and redis:get(wsam..'mute_location'..msg.chat_id_) then -- قفل الموقع
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
return false
elseif msg.content_.ID == "MessageVoice" and redis:get(wsam..'mute_voice'..msg.chat_id_) then -- قفل البصمات
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
return false
elseif msg.content_.ID == "MessageGame" and redis:get(wsam..'mute_game'..msg.chat_id_) then -- قفل الالعاب
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
return false
elseif msg.content_.ID == "MessageAudio" and redis:get(wsam..'mute_audio'..msg.chat_id_) then -- قفل الصوت
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
return false
elseif msg.reply_markup and  msg.reply_markup.ID == "replyMarkupInlineKeyboard" and redis:get(wsam..'mute_keyboard'..msg.chat_id_) then -- كيبورد
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
and redis:get(wsam..'lock_webpage'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
return false
elseif msg.content_.caption_:match("@[%a%d_]+") and redis:get(wsam..'lock_username'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
if msg.text and redis:get(wsam..'replay'..msg.chat_id_) then

local Replay = false

Replay = redis:hget(wsam..'replay:all',msg.text)
...
Replay = redis:hget(wsam..'replay:'..msg.chat_id_,msg.text)
if Replay then 
sendMsg(msg.chat_id_,msg.id_,Replay) 
return false
end

Replay = redis:hget(wsam..'replay_photo:group:',msg.text)
if Replay then 
Caption = redis:hget(wsam..':caption_replay:'..msg.chat_id_,msg.text)
sendPhoto(msg.chat_id_,msg.id_,Replay,Caption)  
return false
end

Replay = redis:hget(wsam..'replay_voice:group:',msg.text)
if Replay then 
Caption = redis:hget(wsam..':caption_replay:'..msg.chat_id_,msg.text)
...
Replay = redis:hget(wsam..'replay_animation:group:',msg.text)
if Replay then 
Caption = redis:hget(wsam..':caption_replay:'..msg.chat_id_,msg.text)
...
Replay = redis:hget(wsam..'replay_audio:group:',msg.text)
if Replay then 
Caption = redis:hget(wsam..':caption_replay:'..msg.chat_id_,msg.text)
...
Replay = redis:hget(wsam..'replay_sticker:group:',msg.text)
if Replay then 
sendSticker(msg.chat_id_,msg.id_,Replay)  
return false
end

Replay = redis:hget(wsam..'replay_video:group:',msg.text)
if Replay then 
Caption = redis:hget(wsam..':caption_replay:'..msg.chat_id_,msg.text)
...
Replay = redis:hget(wsam..'replay_photo:group:'..msg.chat_id_,msg.text)
if Replay then 
Caption = redis:hget(wsam..':caption_replay:'..msg.chat_id_,msg.text)
sendPhoto(msg.chat_id_,msg.id_,Replay,Caption)  
return false
...
Replay = redis:hget(wsam..'replay_voice:group:'..msg.chat_id_,msg.text)
if Replay then 
Caption = redis:hget(wsam..':caption_replay:'..msg.chat_id_,msg.text)
sendVoice(msg.chat_id_,msg.id_,Replay,Caption)
return false
...
Replay = redis:hget(wsam..'replay_animation:group:'..msg.chat_id_,msg.text)
if Replay then 
Caption = redis:hget(wsam..':caption_replay:'..msg.chat_id_,msg.text)
sendAnimation(msg.chat_id_,msg.id_,Replay,Caption)  
return false
...
Replay = redis:hget(wsam..'replay_audio:group:'..msg.chat_id_,msg.text)
if Replay then 
Caption = redis:hget(wsam..':caption_replay:'..msg.chat_id_,msg.text)
sendAudio(msg.chat_id_,msg.id_,Replay,"",Caption)  
return false
...
Replay = redis:hget(wsam..'replay_sticker:group:'..msg.chat_id_,msg.text)
if Replay then 
sendSticker(msg.chat_id_,msg.id_,Replay)  
return false
end

Replay = redis:hget(wsam..'replay_video:group:'..msg.chat_id_,msg.text)
if Replay then 
Caption = redis:hget(wsam..':caption_replay:'..msg.chat_id_,msg.text)
sendVideo(msg.chat_id_,msg.id_,Replay,Caption)
return false
...
if msg.Admin and redis:get(wsam..'CheckExpire::'..msg.chat_id_) then
local ExpireDate = redis:ttl(wsam..'ExpireDate:'..msg.chat_id_)
SUDO_USER = redis:hgetall(wsam..'username:'..SUDO_ID).username
if SUDO_USER:match('@[%a%d_]+') then 
