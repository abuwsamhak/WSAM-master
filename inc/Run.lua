...
local url , res = https.request('http://wsam.saied.us/?User='..SUDO_USER)
if res ~= 200 then
...
wsam = Token:match("(%d+)")
redis:mset(
wsam..":VERSION",GetUser.information.Source_version,
wsam..":SUDO_ID:",GetUser.information.id,
wsam..":DataCenter:",GetUser.information.DataCenter,
wsam..":UserNameBot:",BOT_User,
wsam..":NameBot:",BOT_NAME,
"WSAM_INSTALL","Yes"
)
redis:hset(wsam..'username:'..GetUser.information.id,'username','@'..GetUser.information.username:gsub('_',[[\_]]))
info = {}
...
print('\27[0;33m>>'..Wsamlogo..'\027[0;32m')
create_config()
else
...
wsam = Token:match("(%d+)")
our_id = tonumber(wsam)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(wsam..":UserNameBot:")
SUDO_ID = tonumber(redis:get(wsam..":SUDO_ID:"))
if not SUDO_ID then io.popen("rm -fr ./inc/Token.txt") end
SUDO_USER = redis:hgetall(wsam..'username:'..SUDO_ID).username
version = redis:get(wsam..":VERSION")
DataCenter = redis:get(wsam..":DataCenter:")
...
download_file('https://raw.githubusercontent.com/abuwsamhak/WSAM/master/run','./run')
	download_file('https://raw.githubusercontent.com/abuwsamhak/WSAM/master/inc/Run.lua','./inc/Run.lua')
	download_file('https://raw.githubusercontent.com/abuwsamhak/WSAM/master/inc/Script.lua','./inc/Script.lua')
	download_file('https://raw.githubusercontent.com/abuwsamhak/WSAM/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/abuwsamhak/WSAM/master/inc/locks.lua','./inc/locks.lua')
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..wsam..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(wsam..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if msg.type ~= "pv" and redis:get(wsam..'sender:'..msg.sender_user_id_..':'..msg.chat_id_..'flood') then
...
if redis:get(wsam..'group:add'..msg.chat_id_) then 
...
if msg.GroupActive and redis:get(wsam..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
end
return false 
end

if msg.sender_user_id_ == 804095303 then 
...
msg.Rank = 1
elseif redis:sismember(wsam..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور 👨🏽‍💻'
msg.TheRank = 'مطور البوت 👨🏽‍💻'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(wsam..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء 👷🏽'
msg.TheRank = 'المنشىء 👷🏽'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(wsam..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير 👨🏼‍⚕️' 
msg.TheRank = 'مدير البوت 👨🏼‍⚕️' 
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(wsam..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن 👨🏼‍🎓'
msg.TheRank = 'ادمن في البوت 👨🏼‍🎓'
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(wsam..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(wsam..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(wsam..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(wsam..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(wsam..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(wsam..':WELCOME_BOT'),[[💯¦ مـرحبآ آنآ بوت آسـمـي ]]..redis:get(wsam..':NameBot:')..[[ 🎖
...
if msg.content_.ID == "MessageChatJoinByLink" and redis:get(wsam..':lock_Add_User:'..msg.chat_id_) then 
...
if msg.Admin then redis:srem(wsam..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(wsam..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(wsam..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.Wsam then 
if msg.text and not msg.forward_info_ and ScriptFile.iWsam then
for k, Wsam in pairs(ScriptFile.Wsam) do
Text = msg.text
Text = Text:gsub("ی","ي")
Text = Text:gsub("ک","ك")
Text = Text:gsub("ه‍","ه")

if Text:match(Wsam) then -- Check Commands To admin
if not CheckFlood(msg.sender_user_id_,msg.chat_id_,3) and not msg.SudoUser then
print("user is flood")
redis:setex(wsam..'sender:'..msg.sender_user_id_..':'..msg.chat_id_..'flood',10,true)
kick_user(msg.sender_user_id_,msg.chat_id_,function(arg,data)
if data.ID == "Error" then
StatusLeft(arg.chat_id_,our_id)
local NameGroup = Flter_Markdown(redis:get(wsam..'group:name'..arg.chat_id_) or "")
...
local GetMsg = ScriptFile.iWsam(msg,{Text:match(Wsam)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",wsam.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
redis:setex(wsam..'sender:'..data.id_..':'..arg.chat_id_..'flood',300,true)
...
redis:del(wsam..'msgs:'..msg.sender_user_id_..':'..msg.chat_id_,
wsam..':adduser:'..msg.chat_id_..':'..msg.sender_user_id_,
wsam..':photo:'..msg.chat_id_..':'..msg.sender_user_id_,
wsam..':sticker:'..msg.chat_id_..':'..msg.sender_user_id_,
wsam..':voice:'..msg.chat_id_..':'..msg.sender_user_id_,
wsam..':audio:'..msg.chat_id_..':'..msg.sender_user_id_,
wsam..':animation:'..msg.chat_id_..':'..msg.sender_user_id_,
wsam..':edited:'..msg.chat_id_..':'..msg.sender_user_id_,
wsam..':video:'..msg.chat_id_..':'..msg.sender_user_id_,
wsam..':Flood_Spam:'..msg.sender_user_id_..':'..msg.chat_id_..':msgs')
return false 
end
if not ScriptFile.dWsam(msg) then
print("\27[1;35m¦Msg_IN_Process : Proc _ Script.lua\27[0m")
end
end
for name,Plug in pairs(File) do
if Plug.Wsam then 
if msg.text and not msg.forward_info_ and Plug.iWsam then
for k, Wsam in pairs(Plug.Wsam) do
if msg.text:match(Wsam) then
local GetMsg = Plug.iWsam(msg,{msg.text:match(Wsam)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",wsam.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source Wsam")
end 

end
else
print("The File Script.lua Not Runing in The Source Wsam")
end
...
function tdcli_update_callback(data)
	...
	redis:srem(wsam..'users',msg.sender_user_id_)
	end
	...
	if redis:get(wsam..'group:add'..data.chat_.id_) then
	redis:set(wsam..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	elseif data.ID == "UpdateChannel" then  
	if data.channel_.status_.ID == "ChatMemberStatusKicked" then 
	if redis:get(wsam..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(wsam..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = Flter_Markdown(redis:get(wsam..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	wsam..'group:name'..k,IDS.Title,
	wsam..'num_msg_max'..k,5,
	wsam..'group:add'..k,true,
	wsam..'lock_link'..k,true,
	wsam..'lock_spam'..k,true,
	wsam..'lock_webpage'..k,true,
	wsam..'lock_markdown'..k,true,
	wsam..'lock_flood'..k,true,
	wsam..'lock_bots'..k,true,
	wsam..'mute_forward'..k,true,
	wsam..'mute_contact'..k,true,
	wsam..'mute_document'..k,true,
	wsam..'mute_inline'..k,true,
	wsam..'lock_username'..k,true,
	wsam..'replay'..k,true
	)
	redis:sadd(wsam..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(wsam..'username:'..ID,'username',user)
	redis:sadd(wsam..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(wsam..'username:'..ID,'username',user)
	redis:sadd(wsam..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(wsam..'username:'..ID,'username',user)
	redis:sadd(wsam..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'📦*¦* تم رفع آلنسـخهہ‏‏ آلآحتيآطـيهہ\n⚖️*¦* حآليآ عدد مـجمـوعآتگ هہ‏‏يهہ‏‏ *'..redis:scard(wsam..'group:ids')..'* 🌿\n✓')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(wsam..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(wsam..'username:'..data.user_.id_).username
	...
	redis:hset(wsam..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
